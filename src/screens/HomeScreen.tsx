...
import { testOpenAIConnection } from "../api/testApi";
import {
...
import { Conversation as ConversationType } from "../services/openaiService";

type Props = BottomTabScreenProps<MainTabParamList, "Home">;

type Conversation = ConversationType & { id: string };

export default function HomeScreen({ navigation }: Props) {
  const [conversations, setConversations] = useState<Conversation[]>([]);
  const [refreshing, setRefreshing] = useState(false);
  const { isDark, theme } = useTheme();
  // Use stack navigation for stack actions
  ...
  const testModels = async () => {
    ...
    if (result.success) {
      const availableModels =
        result.results?.filter((r) => r.status === "✅ Available") ?? [];
      ...
      if (unavailableModels.length > 0) {
        ...
        unavailableModels.forEach(
          (m) => (message += `• ${m.model}: ${m.error}\n`)
        );
      }
      ...
      <View style={styles.conversationContent}>
        ...
        <Text
          style={[
            ...
            { color: (theme as any).textSecondary ?? theme.text },
          ]}
          numberOfLines={2}
        >
              ...
              <FlatList
                ...
                refreshControl={
                  <RefreshControl
                    ...
                    onRefresh={onRefresh}
                    tintColor={
                      typeof theme.text === "string" ? theme.text : "#000"
                    }
                  />
                }
              />
